...
if(campype_config_json$MLST$run_mlst == TRUE) {
  if(campype_config_json$MLST$include_cc == TRUE) {
    ...
    MLST <- 
      read_tsv(file = "MLST/MLST.txt", show_col_types = FALSE) %>% #Comprobar cómo queda el archivo cuando include_cc == FALSE
      mutate_all(as.character) %>%
      mutate_if(is.character, ~replace_na(., "Other"))
    Create_DT(x = MLST, filter = "top", caption = '')
  }
...
if(file.exists("Antimicrobial_resistance_genes/AMRFinder/AMR_point_mutations_AMRFinder_matrix.tsv") == FALSE) {
  pander(cat("Resistance caused by point mutations is only included for *Campylobacter* spp. (not for mixing genus)", "  \n"))
  ...
  if('abricate' %in% campype_config_json$antimicrobial_resistance_genes$antimicrobial_resistance_genes_predictor_tool) {
    ...
    for (i in seq_along(DBs)) {
      ...
      if(DBs[i] == "card" | DBs[i] == "ncbi") {
        top_annotation = HeatmapAnnotation(df = resistance_class_annotation, #que solo aparezca esta anotación cuando DBs = card or ncbi.
                                           which = "col", 
                                           simple_anno_size = unit(4, "mm"), 
                                           annotation_name_side = "left", 
                                           gap = unit(1, 'mm'))
      ...
      if(campype_config_json$MLST$run_mlst == TRUE) {
        if(campype_config_json$MLST$include_cc == TRUE) {
          ...
          if (all(Anti_Res_Genes_matrix == 1) == TRUE) { # presence matrix (only 1 values)
            draw(
              Heatmap(as.matrix(Anti_Res_Genes_matrix), 
        ...
        } else { #ERROR: if(campype_config_json$MLST$include_cc == TRUE) { 
          Anti_Res_Genes_matrix <- as.data.frame(Anti_Res_Genes_matrix)
          Anti_Res_Genes_matrix <- tibble::rownames_to_column(Anti_Res_Genes_matrix, "Sample")
  ...
  if('blast' %in% campype_config_json$virulence_genes$virulence_genes_predictor_tool) {
    ...
    if(campype_config_json$MLST$run_mlst == TRUE) {
      ...
      if(campype_config_json$MLST$include_cc == TRUE) {
        ...
        Heatmap(as.matrix(Virulence_Res_Genes_matrix),
...
|--------------------------|-------------|
| Trimmomatic v0.39        | [A.M. Bolger et al., 2014](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4103590/)   |
| Prinseq v0.20.4          | [R. Schmieder and R. Edwards, 2011](https://www.ncbi.nlm.nih.gov/pubmed/21278185)   |
| FLASH v1.2.11            | [T. Magoc and S. Salzberg, 2011](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3198573/) |
| SPAdes v3.14.0           | [A. Bankevich et al., 2012](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3342519/) |
