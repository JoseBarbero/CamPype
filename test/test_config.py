config = {
    "output_directory": "./test/",    # "." is CamPype's directory (you need to have writing permissions to save CamPype's output in certain directories.). Default: "."
    "custom_output_name": "test_results", # Custom name for the output directory. Default: "" (if this parameter is empty: "CamPype_OUTPUT_YYYYMMDD_HHMMSS")
    "n_threads": 4,   # Number of threads to use. Default: None (takes threads defined by default by each tool). Default: 28
    "reference_genome": {   # OPTIONAL (if you don't want to use it, just leave every parameter empty)
        "file": "",    # Required if you want to reorder genome contigs against a reference genome and search for SNPs
        "genus": "",   
        "species": "",    
        "strain": "",
        "proteins": "", # Required if you want to reduce annotation mismatches using PROKKA compared to a reference genome and get detailed information of SNPS. Genome sequence is required at the end of the file for successful execution 
    },
    ...
    "MLST": {
        ...
        "include_cc": False # Will assign Sequence Types (ST) to Clonal Complexes (CC) using the PubMLST Campylobacter jejuni/coli database (internet connection is required). You must set this option to False if you are analysing other genus. Default: True
    },
    "annotation": {
    ...
    "virulence_genes":{
        ...
        "virulence_genes_predictor_tool": ["abricate"], # Select as many tools as desired to identify virulence genes: abricate and/or blast. Keep in mind that abricate uses BLASTn and our BLAST against an inhouse database uses tBLASTn. Default: ["abricate", "blast"].
        "abricate": {
