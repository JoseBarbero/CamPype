...
Virulence_Genes_Screening <- 
  read_tsv(file = "Virulence_genes/BLAST_custom_virulence_genes/Virulence_genes_BLAST_processed.tsv", show_col_types = FALSE) %>%
  relocate(Sample) %>% 
  mutate(across(where(is_character), as_factor)) %>% 
  select(!tail(names(.), 1))
```
...
Virulence_Genes_Blast <- 
  read_tsv(file = "Virulence_genes/BLAST_custom_virulence_genes/Virulence_genes_BLAST_matrix.tsv", show_col_types = FALSE) %>% 
  mutate(across(where(is_character), as_factor)) %>% 
  slice(-n()) %>% 
